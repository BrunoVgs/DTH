...
class HomeController extends AbstractController
{
    ...
    // Decode the JWT token to get the user data (replace this with your actual JWT decoding logic)
    $decodedData = // Decode the JWT token and extract user data (e.g., username, roles)

    // Create a new UsernamePasswordToken with the decoded user data
    $token = new UsernamePasswordToken($decodedData['username'], $JWT, 'main', $decodedData['roles']);

    // Set the token in the token storage
    $tokenStorage->setToken($token);

    // You can now use $this->getUser() in the template and other parts of your application
    // to get the authenticated user object (if needed)
    $user = $this->getUser();
