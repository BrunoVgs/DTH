...
In this code, we create a new UsernamePasswordToken object with the JWT token as the credentials, and then we set this token in the token storage using $tokenStorage->setToken($token). This will store the token in the token storage, and you can now use the user object in the template and other parts of your application.
