...
class JwtAuthenticator extends AbstractGuardAuthenticator
{
    ...
    public function getUser($credentials, UserProviderInterface $userProvider): ?UserInterface
    {
         ...
         try {
             ...
             // Here, we create a new Token with the decoded token payload
             $authToken = new PreAuthenticatedToken(
                 $decodedToken, // The token payload will be used as the user identifier
                 null,
                 'main', // The firewall name
                 []
             );
 
             $user = $userProvider->loadUserByUsername($decodedToken['username']);
             
             // Update the TokenInterface with the authenticated token
             return $authToken;
         } catch (\Exception $e) {
